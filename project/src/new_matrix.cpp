...
static inline void micro_8x8(
    const double* __restrict__ A,  // A + i*n + kk
    const double* __restrict__ B,  // B + kk*n + j
    double*       __restrict__ C,  // C + i*n + j
    int klen, int n)
{
    __m256d c[8][2];
    for (int r = 0; r < 8; r++) {
...
Matrix Matrix::operator*(const Matrix& other) const
{
    const int n  = rows;      // assumes square; adjust if needed
