...
const std::string kernel_source_matrix_mul = R"(
    __kernel void matrix_mul(__global const float* A,
        ...
        float Bpriv[16];
        float Cpriv[16];
        for (int i = 0; i < 16; i++) Cpriv[i] = 0.0f;

        const int TILE = 64; // assume tiles of size 64 each so matrix size is a multiple of 64
        const int SUB_TILE = 4; // 64/16 subtile size
        
        int row;
        int col;

        // Iterate over part of A rows and B cols
        for (int t = 0; t < A_cols / TILE; t++){

            // Inititialize Local blocks (common to work group) Each work item contributes
            for (int j = 0; j < SUB_TILE; j++){
                for (int k = 0; k < SUB_TILE; k++){
                    row = lrow * TILE + prow * SUB_TILE + j;
                    col = t * TILE + pcol * SUB_TILE + k;
                    Aloc[(prow * SUB_TILE + j) * TILE + pcol * SUB_TILE + k] = A[row * A_cols + col];

                    row = t * TILE + prow * SUB_TILE + j;
                    col = lcol * TILE + pcol * SUB_TILE + k;
                    Bloc[(prow * SUB_TILE + j) * TILE + pcol * SUB_TILE + k] = B[row * B_cols + col];
                }
            }
            ...
            for (k = 0; k < TILE; k++){
                ...
                for (int j = 0; j < SUB_TILE; j++){
                    Apriv[j] = Aloc[(prow * SUB_TILE + j) * TILE + k];
                    Bpriv[j] = Bloc[(k) * TILE + pcol * SUB_TILE + j];
                }
                // compute contribution of column k/ row k of A/ B to private tile
                for (int j = 0; j < SUB_TILE; j++){
                    for (int m = 0; m < SUB_TILE; m++){
                        Cpriv[j * SUB_TILE + m] += Apriv[j] * Bpriv[m];
                    }
                }
        ...
        for (int j = 0; j < SUB_TILE; j++){
            for (int k = 0; k < SUB_TILE; k++){
                C[(lrow * TILE + prow * SUB_TILE + j) * B_cols + lcol * TILE + pcol * SUB_TILE + k] = Cpriv[j * SUB_TILE + k];
            }
        }
    }
)";
...
MatrixCL MatrixCL::operator*(const MatrixCL& other) const
{
    ...
    current_mat_mul.setArg(5, rows_);
    current_mat_mul.setArg(6, cols_);
    current_mat_mul.setArg(7, other.cols_);

    
    cl::NDRange global(C_rows / SUB_TILE, C_cols / SUB_TILE);  // one work item per SUB_TILE×SUB_TILE output
    cl::NDRange local(TILE / SUB_TILE, TILE / SUB_TILE);        // 16×16 = 256 work items per group
    queue_.enqueueNDRangeKernel(current_mat_mul, cl::NullRange, global, local);
